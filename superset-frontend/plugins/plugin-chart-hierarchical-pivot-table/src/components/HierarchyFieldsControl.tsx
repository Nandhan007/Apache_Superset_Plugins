...
export default function HierarchyFieldsControl({
  value = [],
  onChange,
  datasourceColumns = [],
}: HierarchyFieldsControlProps) {
  ...
  const handleEdit = (index: number) => {
    ...
    const formValues = {
      ...
      parentField: item.parentField
        ? Array.isArray(item.parentField)
          ? item.parentField
          : [item.parentField]
        : [],
      filterColumn: item.filterColumn
        ? Array.isArray(item.filterColumn)
          ? item.filterColumn
          : [item.filterColumn]
        : [],
    };
  ...
  const groupedItems = value.reduce(
    (acc, item, index) => {
    ...
    {} as Record<
      string,
      { item: HierarchyFieldConfig; originalIndex: number }[]
    >,
  );
  ...
  return (
    <div>
      ...
      >
        {/* <Typography.Text strong>{t('Hierarchy Configuration')}</Typography.Text> */}
        <div>
      ...
      {isJsonMode ? (
        ...
        <div>
          ...
          >
            {t(
              'Define hierarchy levels via a structured JSON array. Each object requires level, fieldName, fieldLabel, columnName, and hierarchyGroup.',
            )}
          </Typography.Paragraph>
          <div
            style={{
            ...
            <div key={group} style={{ marginBottom: 16 }}>
              ...
              <List
                ...
                renderItem={({ item, originalIndex }) => (
                  ...
                  >
                    <List.Item.Meta title={item.fieldLabel} />
                  </List.Item>
                )}
              />
            </div>
          ...
          >
            <Form form={form} layout="vertical">
              ...
              <Form.Item
                name="level"
                label={t('Level')}
                rules={[{ required: true, message: t('Level is required') }]}
              >
                <InputNumber min={1} style={{ width: '100%' }} />
              </Form.Item>
              <Form.Item
                name="fieldName"
                label={t('Field Name (Internal ID)')}
                rules={[{ required: true }]}
              >
                <Select showSearch placeholder="Select column">
              ...
              <Form.Item
                ...
                shouldUpdate={(prevValues, currentValues) =>
                  ...
                  prevValues.hierarchyGroup !== currentValues.hierarchyGroup
                }
              >
                {({ getFieldValue }) => {
                  const currentLevel = Number(getFieldValue('level')) || 0;
                  const currentGroup =
                    getFieldValue('hierarchyGroup') || 'Default';

                  return (
                    <Form.Item
                      name="parentField"
                      label={t('Parent Field(s)')}
                      tooltip={t(
                        'ID of parent field(s). Only fields from the SAME group with a LOWER level can be selected.',
                      )}
                    >
                      <Select
                        mode="tags"
              ...
              <Form.Item
                name="filterColumn"
                label={t('Filter Column(s)')}
                tooltip={t(
                  'Column in THIS table to filter by parent value. Comma separated if multiple parents.',
                )}
              >
                <Select
                  showSearch
              ...
              <Form.Item
                ...
                tooltip={t(
                  'How to sort values in this level. Use Chronological for Months, Days, Quarters, etc.',
                )}
              >
                <Select>
                  <Select.Option value="Default">
                    {t('Default (Alphabetical/Numeric)')}
                  </Select.Option>
                  <Select.Option value="Chronological">
                    {t('Chronological (Time-based)')}
                  </Select.Option>
                </Select>
              </Form.Item>
