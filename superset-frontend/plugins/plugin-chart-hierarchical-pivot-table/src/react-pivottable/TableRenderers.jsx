...
export const TableRenderer = React.memo(props => {
  ...
  useEffect(() => {
    ...
    return () => cellEditManager.removeChangeListener(handleChange);
  ...
  const getUniqueValues = useCallback(
    key => {
      ...
      // Check if key is a dimension in data
      // We iterate data and check visibility
      const otherFiltersEntries = Object.entries(filters).filter(
        ([k, v]) => k !== key && v && v.length > 0,
      );
      ...
      if (hasMetricFilters && cachedBasePivotSettings) {
        const { rowKeys, colKeys, pivotData, rowAttrs } =
        ...
        if (targetRowAttrIdx !== -1) {
          ...
          const otherDimensionFilters = otherFiltersEntries.filter(
            ([k]) => data[0][k] !== undefined,
          );
          const otherMetricFilters = otherFiltersEntries.filter(
            ([k]) => data[0][k] === undefined,
          );
      ...
      while (i < visibleColKeys.length) {
        ...
        if (attrIdx < colKey.length) {
          ...
          const handleSort = e => {
            ...
            if (e.shiftKey) {
              ...
              if (existingSortIndex !== -1) {
                ...
                if (currentDir === 'asc')
                  newSortModel[existingSortIndex].direction = 'desc';
              ...
              if (existingSortIndex !== -1 && sortModel.length === 1) {
                ...
                else if (sortModel[0].direction === 'desc')
                  nextDirection = null;
              }
          ...
          const sortOrderIndicator =
            sortModel.length > 1 && isSorted && isLastLevel ? (
                  ...
                  >
                    <FilterOutlined
                      onClick={e => {
                        e.stopPropagation();
                        setActiveFilterMenu(
                          activeFilterMenu === fKey ? null : fKey,
                        );
                      }}
                      style={{
                        marginLeft: 5,
                        color:
                          filters[fKey] && filters[fKey].length > 0
                            ? '#1890ff'
                            : undefined,
                      }}
                    />
    ...
    pivotSettings => {
      ...
      return (
        <tr key="rowHdr">
          ...
          {hasRedirection && (
            <th className="pvtAxisLabel">{t('Redirection')}</th>
          )}
          {rowAttrs.map((r, i) => {
            ...
            const sortOrderIndicator =
              sortModel.length > 1 && isSorted ? (
                <span
                  style={{ fontSize: '10px', marginLeft: 2, marginRight: 2 }}
                >
                  {sortIndex + 1}
                </span>
              ) : null;
  ...
  const renderTableRow = useCallback(
    (rowKey, rowIdx, pivotSettings) => {
      ...
      if (hasRowActions) {
        if (
          rowAttrs.length > 0 &&
          rowAttrs[actionRowSpanIndex] === 'Metric' &&
          rowAttrs.length > 1
        ) {
          ...
          return (
            <th
              ...
              style={{
                ...
                ...(!isFirstInGroup &&
                hasRowActions &&
                tableOptions.enableRowGrouping !== false &&
                i <= actionRowSpanIndex
                  ? { borderTop: 'none' }
                  : {}),
              }}
            >
  ...
  const visibleRowKeys = useMemo(() => {
    ...
    return visibleRowKeysRaw.filter(rowKey => {
      ...
      return visibleColKeys.every(colKey => {
        ...
        const agg = cachedBasePivotSettings.pivotData.getAggregator(
          rowKey,
          colKey,
        );
        const val = agg.value();
