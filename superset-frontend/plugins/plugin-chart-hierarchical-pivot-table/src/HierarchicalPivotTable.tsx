...
import { MinusSquareOutlined, PlusSquareOutlined, TableOutlined } from '@ant-design/icons';
...
import { aggregatorTemplates, PivotTable, sortAs, getCustomSortKey, naturalSort } from './react-pivottable';
...
export default function HierarchicalPivotTable(props: PivotTableProps) {
  ...
  const handleFormSubmit = async (formData: Record<string, any>) => {
      ...
      try {
          ...
          const containsFile = Object.values(formData).some(val => 
          ...
          setTimeout(async () => {
              ...
              const store = typeof window !== 'undefined' ? (window as any).store : null;
              if (store && store.dispatch && store.getState) {
                  ...
                  if (state.charts) {
                      Object.keys(state.charts).forEach(chartIdStr => {
                          ...
                          if (!isNaN(chartId)) {
                              store.dispatch({ type: 'TRIGGER_QUERY', value: Date.now(), key: chartId });
                          }
                      });
                  }
  ...
  const sorters = useMemo(
    () => {
      ...
      const isTimeDimension = (name: string) => {
          return /year|month|quarter|half|season|week|day/i.test(name.toLowerCase());
      };
      ...
      hierarchyFields.forEach(field => {
          ...
          if (field.sortMethod === 'Chronological') {
              ...
              if (isTimeDimension(field.fieldLabel) || isTimeDimension(field.columnName)) {
                 useChrono = true;
              }
          }
          ...
          if (useChrono) {
               ...
               const colDef = fetchedColumns.find(c => c.column_name === field.columnName) || 
               ...
               timeSorters[field.columnName] = (a, b) => {
                   return naturalSort(getCustomSortKey(a, true), getCustomSortKey(b, true));
               };
               ...
               if (field.fieldLabel && field.fieldLabel !== field.columnName && field.fieldLabel !== verboseName) {
    ...
    [metricNames, hierarchyFields, fetchedColumns, allColumns, layoutRows, layoutCols],
  );
  ...
  return (
    <Styles height={height} width={width} margin={theme.sizeUnit * 4}>
      <Spin spinning={isSaving || isRefreshing} tip={isSaving ? t('Saving changes...') : t('Loading...')} style={{ height: '100%', width: '100%' }}>
        <div ref={containerRef} style={{ height: '100%', width: '100%', paddingTop: '10px', position: 'relative' }}>
        <Modal
        ...
        <div className="editable-pivot-table-toolbar" style={{ position: 'absolute', top: '-16px', right: 0, zIndex: 10, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '8px' }}>
          ...
          {enableLayout && (
          <Button size="small" onClick={() => setIsLayoutEditorVisible(true)} className='editable-pivot-table-layout-btn pivot-editable-table-layout-button' icon={<TableOutlined />} disabled={isRefreshing}>Layout</Button>
          )}
        </div>
        <LayoutEditor
        ...
        <PivotTableWrapper className="scrollable">
          <PivotTable
            ...
            onRegisterSave={(saveFn: () => Promise<boolean | void> | void) => {
            ...
            onFilterChange={(filters: any) => {
               setHasActiveFilters(filters && Object.values(filters).some((v: any) => v && v.length > 0));
            }}
