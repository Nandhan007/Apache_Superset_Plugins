  ...
  describe('transformProps', () => {
    ...
    describe('TableChart', () => {
      ...
      describe('Redirection configuration', () => {
        ...
        it('should append uniqueField to row-level redirection targetUrl', () => {
          const props = transformProps({
            ...testData.basic,
            rawFormData: {
          ...
          // Find the menu trigger button and click it to open popover
          const popoverBtns = screen.getAllByRole('button');
          const popoverBtn = popoverBtns.find(btn => btn.querySelector('.anticon-menu'));
          expect(popoverBtn).toBeDefined();
          popoverBtn?.click();

          // Click the redirection menu item
          const menuItem = screen.getByText('Redirect Unique Row');
          expect(menuItem).toBeInTheDocument();
          menuItem.click();

          expect(window.location.href).toBe('https://example.com/item/Michael');
        });

        it('should append uniqueField to global redirection targetUrl using first selected row', () => {
          const props = transformProps({
            ...testData.basic,
            rawFormData: {
              ...testData.basic.rawFormData,
          ...
          // Select the first row to establish selectedRowData
          const inputs = document.querySelectorAll('input');
          const checkbox = Array.from(inputs).find(input => input.type === 'radio' || input.type === 'checkbox');
          expect(checkbox).toBeDefined();
          checkbox?.click();

          // Find global redirection button and click it
          const globalBtn = screen.getByText('Global Redirect Unique');
          expect(globalBtn).toBeInTheDocument();
          globalBtn.click();

          expect(window.location.href).toBe('https://example.com/batch/Michael');
        });
      });
    });
  });
