...
const processColumns = memoizeOne(function processColumns(
  ...
  const {
    ...
    rawFormData: {
      ...
      query_mode: queryModeSetting,
      all_columns: allColumns_,
      groupby: groupby_,
      rowLevelActions,
    },
    queriesData,
  ...
  const effectiveMetrics = props.ownState?.metrics || metrics_;
  const metrics = (effectiveMetrics ?? []).map(getMetricLabel);
  ...
  const uniqueFields = [
    ...(rowLevelActions || []).map((action: any) => action.uniqueField),
    ...(props.rawFormData?.htmlViewerActions || []).map(
      (action: any) => action.uniqueField,
    ),
    ...(props.rawFormData?.redirectionUrls || []).map(
      (redr: any) => redr.uniqueField,
    ),
    ...(props.rawFormData?.globalRedirectionUrls || []).map(
      (redr: any) => redr.uniqueField,
    ),
  ...
  const isUniqueFieldHidden = (key: string) => {
    ...
    if (queryMode === QueryMode.Raw) {
      return !ensureIsArray(allColumns_).some(col =>
        typeof col === 'string' ? col === key : col?.label === key,
      );
    }
    return !ensureIsArray(groupby_).some(col =>
      typeof col === 'string' ? col === key : col?.label === key,
    );
  };
