...
const SingletonDndProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  return (
    <DndProvider backend={HTML5Backend} options={{ rootElement: window }}>
      <ManagerRegistrar>{children}</ManagerRegistrar>
    </DndProvider>
  );
};

// Helper to capture the manager ref and put it on window
const ManagerRegistrar: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
...
interface CardProps {
  ...
  moveCard: (
    dragIndex: number,
    hoverIndex: number,
    sourceList: 'rows' | 'cols' | 'metrics',
    targetList: 'rows' | 'cols' | 'metrics',
  ) => void;
  toggleSelection: (id: string, listType: 'rows' | 'cols' | 'metrics') => void;
}

const Card: React.FC<CardProps> = React.memo(
  ({ id, text, index, moveCard, listType, isSelected, toggleSelection }) => {
...
export default function LayoutEditor({
  ...
  const moveCard = useCallback(
    (
      dragIndex: number,
      hoverIndex: number,
      sourceList: 'rows' | 'cols',
      targetList: 'rows' | 'cols',
    ) => {
      setItems(prevItems => {
  ...
  const filteredMetrics = metricItems.filter(
    m =>
      !searchText ||
      m.metric_name.toLowerCase().includes(searchText.toLowerCase()) ||
      (m.verbose_name &&
        m.verbose_name.toLowerCase().includes(searchText.toLowerCase())),
  );
